...
hotel_booking_prompt = ChatPromptTemplate.from_messages(
    [
        (
            ...
            "用户提供 hotel_id 并明确要求预订时，必须立即调用 book_hotel；hotel_id 是产品 ID，不能传给 fetch_order_detail。"
            "只有用户提供正式 order_id 或明确询问自己的已有订单时，才调用订单查询工具。"
            "搜索时要保持耐心，如果第一次没有查到结果，请适当扩大查询范围继续尝试。"
            "\n\n关于取消请求：\n"
            "- 当用户说“取消它”“取消我的预订”等类似表达时，请结合对话历史判断用户指的是哪一家酒店。\n"
            "- 订单状态和明细必须使用 fetch_user_orders 或 fetch_order_detail 从 PostgreSQL 查询。\n"
            "- 如果最近提到过某张酒店订单，请优先使用正式 order_id 执行取消。\n"
            "- 处理取消请求时，必须调用 cancel_hotel 工具，不能只回复一段文字。\n"
            "- cancel_hotel 工具需要提供正式 order_id 参数。\n"
            ...
            " - “取消它”（指已有预订）→ 调用 cancel_hotel 并传入 order_id\n"
            " - “取消我的酒店预订” → 先查询订单，再调用 cancel_hotel\n"
            " - “我想取消这个订单” → 调用 cancel_hotel 并传入 order_id\n"
            " - “请帮我取消酒店” → 先查询订单，再调用 cancel_hotel\n"
            "处理取消时，你应当尽量从上下文中识别 order_id。"
...
# 酒店助手工具
book_hotel_safe_tools = [search_hotels, fetch_user_orders, fetch_order_detail, CompleteOrEscalate]
